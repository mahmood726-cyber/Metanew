...
async def validate_data(
    ...
    try:
        logger.info(f"Validation request from user: {current_user.username if current_user else 'anonymous'}")
...
async def compute_effect_sizes(
    ...
    try:
        logger.info(f"Effect size computation from user: {current_user.username if current_user else 'anonymous'}")
...
async def run_psa(
    ...
    """
    try:
        logger.info(f"PSA request from user: {current_user.username if current_user else 'anonymous'}")
